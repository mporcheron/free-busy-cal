...
class FreeBusyCalendar extends \ArrayObject
{
     ...
     */
    public function merge(FreeBusyCalendar $cal)
    {
    ...
    {
        foreach ($arr as $date => $times) {
            if (!$this->offsetExists($date)) {
                $this->offsetSet($date, $times);
                continue;
            }

            $this_times = $this->offsetGet($date);
            foreach ($times as $hour => $minutes) {
                foreach ($minutes as $minute => $available) {
                    if (!$available) {
                        $this_times[$hour][$minute] = false;
                    }
                }
            }
            $this->offsetSet($date, $this_times);
        }
    }
}
